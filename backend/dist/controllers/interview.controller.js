...
export const createInterview = asyncHandler(async (req, res) => {
    ...
    const session = await interviewService.createSession(req.user.id, title, type);
    res.status(201).json({
        success: true,
...
export const startInterview = asyncHandler(async (req, res) => {
    ...
    const result = await interviewService.startInterview(req.params.id, req.user.id);
    res.status(200).json({
        success: true,
...
export const answerInterviewQuestion = asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ApiError(401, "Authentication required");
    }
    ...
    const result = await interviewService.answerQuestion(req.params.id, req.user.id, answer);
    res.status(200).json({
        success: true,
