...
class InterviewService {
    async createSession(userId, title, type) {
...
`.trim();
        const question = (await aiService.generate(prompt)).trim() ||
    ...
    async answerQuestion(sessionId, userId, answer) {
        const session = await prisma.interviewSession.findFirst({
            where: {
                id: sessionId,
                userId,
        ...
        if (!currentQuestion) {
            throw new ApiError(400, "No pending interview question.");
        }
...
`.trim();
        ...
        console.log(result);
        if (result.technicalScore === undefined ||
            result.communicationScore === undefined ||
            result.confidenceScore === undefined ||
            !result.nextQuestion) {
            throw new ApiError(500, "Invalid AI evaluation response.");
        }
        ...
        const saved = await prisma.interviewQuestion.create({
            data: {
                sessionId,
